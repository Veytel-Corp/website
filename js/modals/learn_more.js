(function loadLearnMoreModalHeaderContent(
  filePath = "./css/modals/learnMore.css"
) {
...
(function loadLearnMoreModal(filePath = "./html/modals/learnMore.html") {
  fetch(filePath)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          `Something went wrong when attempting to get the under learn more modal content. Check file path. ${response.statusText}`
        );
      }
    ...
    .catch((error) => {
      console.error(
        `Something went wrong when fetching the learn more modal content: `,
        error
      );
    });
})();
