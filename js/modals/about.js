...
(function loadAboutModal(filePath = "./html/modals/about.html") {
  fetch(filePath)
    .then((response) => {
      if (!response.ok) {
        throw new Error(
          `Something went wrong when attempting to get the under about modal content. Check file path. ${response.statusText}`
        );
      }
    ...
    .catch((error) => {
      console.error(
        `Something went wrong when fetching the about modal content: `,
        error
      );
    });
})();
